...
CREATE FUNCTION public.querydonations() RETURNS TABLE(p_name character varying, p_iduser text, p_idpayment character varying, p_idfrecuency character varying, p_idamount money)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY SELECT
...
CREATE FUNCTION public.queryiddonations(p_iddonation integer) RETURNS TABLE(p_name character varying, p_iduser text, p_idpayment character varying, p_idfrecuency character varying, p_idamount money)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY SELECT
...
CREATE FUNCTION public.queryidlocations(p_idlocations integer) RETURNS TABLE(p_latitude numeric, p_longitude numeric, p_namelocation character varying, p_idreport character varying)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY SELECT
...
CREATE FUNCTION public.querylocations() RETURNS TABLE(p_latitude numeric, p_longitude numeric, p_namelocation character varying, p_idreport character varying)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY SELECT
...
--
-- Name: querylocationsreport(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.querylocationsreport() RETURNS TABLE(p_report character varying, p_datetime timestamp without time zone, p_deforest character varying, p_affectation character varying, p_description character varying, p_city character varying, p_addres character varying, p_latitude numeric, p_longitude numeric, p_namelocation character varying)
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- Cuerpo de la función
    -- Lógica de la función almacenada
    -- Puedes realizar consultas, manipulación de datos, etc.
    ...
    RETURN QUERY SELECT
        ...
        loca.latitude as Latitude,
        loca.longitude as Longitude,
        loca."nameLocation" as NameLocation
    ...
    JOIN "GradeAffectation" ga ON R."idAffectation" = ga."idAffectation"
    JOIN "City" c ON R."idCity" = c."idCity"
    JOIN "locations" loca ON loca."idReport" = R."idReport"
    ORDER BY R."idReport" ASC;
END;
$$;


ALTER FUNCTION public.querylocationsreport() OWNER TO postgres;
...
61	Software Engineer	abcdefghijklnmopqrstuv	2023-05-11 00:00:00	2023-05-11 00:00:00	t	3
67	Developer	nivel up	2022-12-11 00:00:00	2021-10-11 00:00:00	t	8
\.
...
33	Incendio en la zona montañoza	2019-08-19 00:00:00	7	5	14
\.
...
COPY public."User" ("idUser", "First_Name", "Second_Name", "First_surname", "Second_surname", "Email", "Password", "idCity", "idPhone", "idGender", "idUser_type", active, activation_token, reset_password_token, created_on) FROM stdin;
...
COPY public."user" (id, name, last_name, city, address, phone, gender, email, password, active, activation_token, reset_password_token, created_on) FROM stdin;
\.
...
SELECT pg_catalog.setval('public."Affectation_idAffectation_seq"', 56, true);
...
SELECT pg_catalog.setval('public."Amount_idAmount_seq"', 33, true);
...
SELECT pg_catalog.setval('public."City_idCity_seq"', 32, true);
...
SELECT pg_catalog.setval('public."Deforest_type_idDeforest_seq"', 11, true);
...
SELECT pg_catalog.setval('public."Frecuency_idFrecuency_seq"', 5, true);
...
SELECT pg_catalog.setval('public."Payment_idPayment_seq"', 4, true);
...
SELECT pg_catalog.setval('public."Phone_idPhone_seq"', 31, true);
...
SELECT pg_catalog.setval('public."Report_idReport_seq"', 34, true);
...
SELECT pg_catalog.setval('public."blogs_idBlogs_seq"', 68, true);
...
SELECT pg_catalog.setval('public."entity_user_idEntityus_seq"', 32, true);
...
SELECT pg_catalog.setval('public.locations_idlocations_seq', 19, true);
...
SELECT pg_catalog.setval('public."user_type_idUser_type_seq"', 4, true);
...
--
-- Name: Report Report_Deforest_type_idDeforest_fk; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--
